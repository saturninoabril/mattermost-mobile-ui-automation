...
module.exports = function(driver, t) {
    t.test('Select server URL', async function(t) { // eslint-disable-line
        ...
        const localInstance = 'http://cypress.test.mattermost.com';
        await serverUrlInput.sendKeys(localInstance);
        t.pass('I can type into "Server URL input"');
        const textInput = await serverUrlInput.text();
        t.equal(textInput, localInstance, '> should match text input');

        await connectAction.perform();
        t.ok(
            await driver.waitForElementByAndroidUIAutomator(
                'new UiSelector().text("Connecting...")'
            ),
            '> should match button text as "Connecting..."',
        );
    ...
    t.test('User login', async function(t) { // eslint-disable-line
        ...
        await emailAddressInput.sendKeys('user-1');
        t.pass('I can type into "Email address input"');
        const emailTextInput = await emailAddressInput.text();
        t.equal(emailTextInput, 'user-1', '> should match text input');

        await passwordInput.sendKeys('user-1');
